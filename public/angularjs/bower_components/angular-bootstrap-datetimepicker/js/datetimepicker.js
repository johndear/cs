  ...
  angular.module('ui.bootstrap.datetimepicker', [])
    ...
    .directive('datetimepicker', ['$log', 'dateTimePickerConfig', function datetimepickerDirective($log, defaultConfig) {
      ...
      var validateConfiguration = function validateConfiguration(configuration) {
        var validOptions = ['startView', 'minView', 'minuteStep', 'dropdownSelector', 'todayBtn', 'tomorrowBtn', 'clearBtn'];
      ...
      return {
        ...
        '               class="day" ' +
        '               data-ng-class="{today: dateObject.is_today, active: dateObject.active, past: dateObject.past, future: dateObject.future, disabled: !dateObject.selectable}" >{{ dateObject.display }}</td>' +
        '       </tr>' +
        '   </tbody>' +
        '   <tfoot>' +
        '       <tr><td colspan="7"><table style="width: 100%;"><tr>' +
        '       <th ng-show="todayBtn" style=" padding: 5px 0px;" data-ng-click="changeToToday(data.nextView)" >今天</th>' +
        '       <th ng-show="tomorrowBtn" data-ng-click="changeToTomorrow(data.nextView)">明天</th>' +
        '       <th ng-show="clearBtn" data-ng-click="clearDate()">清除</th>' +
        '      </tr></table></td></tr>' +
        '   </tfoot>' +
        '</table></div>',
        scope: {
        ...
        link: function link(scope, element, attrs, ngModelController) {
            ...
            var is_today = function (somedate) {
                return is_same_day(today, somedate);
            };
            var get_tomorrow = function() {
                return moment.utc(today).add(1, 'days');
            }
          ...
          scope.changeView = function changeView(viewName, dateObject, event) {
            ...
            scope.changeToTomorrow = function (nextView) {
                scope.changeView(nextView, new DateObject({dateValue: get_tomorrow()}));
            };
