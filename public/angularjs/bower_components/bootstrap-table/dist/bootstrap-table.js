...
!function ($) {
    ...
    var BootstrapTable = function (el, options) {
    	//把系统参数，登录用户信息传进来
    	if(options.sysparam)
    		BootstrapTable.DEFAULTS.sysparam = options.sysparam;
    	if(options.userInfo)
    		BootstrapTable.DEFAULTS.userInfo = options.userInfo;
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            //系统参数翻译用	廖金洪
            if(column.gcode && that.options.sysparam && !column.formatter){
            	that.header.formatters.push(function(value,row){
            		var gcodes = that.options.sysparam[column.gcode];
                	for (var key in gcodes){
                		if(value == gcodes[key].mcode)
                			return gcodes[key].mname;
                	}
            	});
        	}else if(column.type=='money'){
        		// 金额默认显示两位小数 - 黄玉云
        		that.header.formatters.push(moneyFormatter);
        	}else{
        ...
        // 恢复默认显示
        if($.isEmptyObject(that.options.defaultVisible)){
        	$.extend(that.options.defaultVisible,visibleColumns);
        }
     
        $.extend(that.options.visibleColumns,visibleColumns);
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showColumns) {
            ...
            html.push(sprintf('<li>' +
                    '<label><input type="button" class="btn btn-primary btn-xs" data-field="%s" value=" %s"></label>' +
                    '</li>', "showAll", "默认显示"));
        ...
        if (this.options.showColumns) {
            ...
            $keepOpen.find('input').off('click').on('click', function () {
                ...
                var $this = $(this);
                // 恢复默认显示列 
                if($(this).data('field')=='showAll'){
                	
                	var inputs = $keepOpen.find('input');
                	$.each(that.options.columns, function (i, column) {
                		if($.inArray(column,that.options.defaultVisible)==-1 && column.visible){
                			column.visible = false ;
                			inputs[i].checked=false;
                			that.toggleColumn(i, false, false);
                        	that.trigger('column-switch', column.field, false);
                		}else if($.inArray(column,that.options.defaultVisible)!=-1 && !column.visible){
                			column.visible = true ;
                			inputs[i].checked=true;
                			that.toggleColumn(i, true, false);
...
//                    style = "style=\"text-align: center;white-space:nowrap\"";
                    text = that.options.cardView ?
                        ['<div class="card-view">',
    ...
    BootstrapTable.prototype.initServer = function (silent, query) {
        ...
        if(this.options.advancedSearch){
        	$.extend(data, this.options.moreConditions);
        }
        ...
        if (data === false) {
        	...
        	// 修改，导出 - 黄玉云
        	this.options.allQueryParams = data;
        }
    ...
    $.fn.bootstrapTable.sprintf = sprintf;
    $.fn.bootstrapTable.calculateObjectValue = calculateObjectValue;
